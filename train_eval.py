...
#gaidong
@torch.no_grad()
def embeddings_create(model,dataset,device): 
    model.eval()
    print(dataset[0])
    embeddings=np.array([])
    biaozhi=False
    for data in dataset:
        data=data.to(device)
        if biaozhi == False:
            print(model(data).is_cuda)
            embeddings=model(data).detach().cpu().numpy()[0]
            biaozhi=True
            continue
        embedding=model(data).detach().cpu().numpy()[0]
        embeddings=np.vstack((embeddings,embedding))
        print(embedding)
    print(embeddings)
    path_temp = r'C:\Users\28341\Desktop\GIB_work\code\AutoGIB\semi_supervised\embeddings'  #文件目录    
    file_name = osp.join(path_temp, "embedding.txt")
    np.savetxt(file_name,embeddings)
...
def joint_cl_exp(
    ...
    for fold, (train_idx, test_idx, val_idx, semi_idx) in enumerate(
        ...
        train_dataset = dataset[train_idx]
        test_idx=test_idx.type(torch.int64) #gaidong
        print(test_idx.dtype) #gaidong
        test_dataset = dataset[test_idx]
        # val_dataset = dataset[val_idx]
        semi_dataset = dataset[semi_idx]

        logger.info(test_dataset.data.y.shape[0])#gaidong
        ...
        for epoch in range(1, epochs + 1):
            ...
            if epoch % lr_decay_step_size == 0:
                for param_group in view_optimizer.param_groups:
                    param_group['lr'] = lr_decay_factor * param_group['lr']
        #embeddings_create(model,dataset,device)#gaidong
